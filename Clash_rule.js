...
// v6.1 更新：
// 1. 增加 ChinaMax 开关
// 2. ChinaMax 一个开关同时控制 ChinaMax_Domain + ChinaMax_IP
// 3. ChinaMax false 时不生成 Provider + RULE-SET
// 4. 保留 ChinaIP 作为固定兜底规则
// 5. 修复部分 URL / 字符串格式问题
//
...
function main(params) {
    ...
    const RULE_OPTIONS = {
        ...
        ChinaMax: true,         // ChinaMax 中国大陆域名 + IP
    ...
    // ============================================================
    // 清理 Nameserver Policy
    // ============================================================

    for (
        const k of Object.keys(subPolicy)
    ) {
    ...
    Object.keys(
        ...
        if (name === "ChinaMax") {
            ...
            params["rule-providers"]
                ["ChinaMax_Domain"] =
                    PROVIDERS["ChinaMax_Domain"];


            params["rule-providers"]
                ["ChinaMax_IP"] =
                    PROVIDERS["ChinaMax_IP"];
    ...
    const regions = [
        {
            ...
            regex:
                "(?i)(美国|美國|洛杉矶|洛杉磯|圣何塞|聖何塞|硅谷|矽谷|西雅图|西雅圖|纽约|紐約|🇺🇸|(^|[^A-Za-z])US([^A-Za-z]|$)|(^|[^A-Za-z])USA([^A-Za-z]|$)|United[ -]?States)",

            icon:
                "https://testingcf.jsdelivr.net/gh/HatScripts/circle-flags@gh-pages/flags/us.svg"
        }
        {
            ...
            regex:
                "(?i)(香港|🇭🇰|(^|[^A-Za-z])HK([^A-Za-z]|$)|(^|[^A-Za-z])HKG([^A-Za-z]|$)|Hong[ -]?Kong)",

            icon:
                "https://testingcf.jsdelivr.net/gh/HatScripts/circle-flags@gh-pages/flags/hk.svg"
        },
...

        {
            ...
            regex:
                "(?i)(日本|东京|東京|大阪|🇯🇵|(^|[^A-Za-z])JP([^A-Za-z]|$)|(^|[^A-Za-z])JPN([^A-Za-z]|$)|Japan)",

            icon:
                "https://testingcf.jsdelivr.net/gh/HatScripts/circle-flags@gh-pages/flags/jp.svg"
        },
...

        {
            ...
            regex:
                "(?i)(韩国|韓国|南韩|南韓|首尔|首爾|🇰🇷|(^|[^A-Za-z])KR([^A-Za-z]|$)|(^|[^A-Za-z])KOR([^A-Za-z]|$)|Korea)",

            icon:
                "https://testingcf.jsdelivr.net/gh/HatScripts/circle-flags@gh-pages/flags/kr.svg"
        },
...

        {
            ...
            regex:
                "(?i)(新加坡|狮城|獅城|🇸🇬|(^|[^A-Za-z])SG([^A-Za-z]|$)|(^|[^A-Za-z])SGP([^A-Za-z]|$)|Singapore)",

            icon:
                "https://testingcf.jsdelivr.net/gh/HatScripts/circle-flags@gh-pages/flags/sg.svg"
        },
    ];
    ...
    apps.forEach(app => {
        ...
        if (app.name === "AI") {
            enabled =
                RULE_OPTIONS.AI === true;
        }


        if (app.name === "Apple") {
            enabled =
                RULE_OPTIONS.Apple === true;
        }
        ...
        if (app.name === "Google") {
            enabled =
                RULE_OPTIONS.Google === true;
        }


        if (app.name === "Microsoft") {
            enabled =
                RULE_OPTIONS.Microsoft === true;
        }


        if (app.name === "Telegram") {
            enabled =
                RULE_OPTIONS.Telegram === true;
        }


        if (app.name === "TikTok") {
            enabled =
                RULE_OPTIONS.TikTok === true;
        }
        ...
        if (app.name === "YouTube") {
            enabled =
                RULE_OPTIONS.YouTube === true;
        }


        if (app.name === "FCM") {
            enabled =
                RULE_OPTIONS.FCM === true;
        }
        ...
        if (app.name === "Instagram") {
            enabled =
                RULE_OPTIONS.Instagram === true;
        }


        if (app.name === "PikPak") {
            enabled =
                RULE_OPTIONS.PikPak === true;
        }


        if (app.name === "Spotify") {
            enabled =
                RULE_OPTIONS.Spotify === true;
        }


        if (app.name === "AdBlock") {
            enabled =
                RULE_OPTIONS.AdBlock === true;
        }
